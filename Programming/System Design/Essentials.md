...
Теорема утверждает, что в случае разделения сети(`Р`) в распределенной компьютерной системе нужно выбирать между доступностью(`А`) и согласованностью(`С`) (как и в теореме CAP),  а в остальных случаях(`Е`), даже когда система работает нормально без разделений, нужно выбирать между задержкой(`L`) и согласованностью(`С`).
