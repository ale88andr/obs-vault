...
Пример `rebase` - изменение *commit-messages*
...
- `edit` — остановиться на этом коммите и внести изменения
- `squash` — объединить с предыдущим коммитом
- `fixup` — как squash, но без сохранения сообщения
- `drop` — удалить коммит
...
При этом стоит учесть, что **поменялись хэши** коммитов!

##### **Rebase с конфликтами и их разрешением**

**Сценарий**: При rebase возникает конфликт, который мы должны вручную разрешить.

```bash
git rebase main
# Git сообщает о конфликте
# Редактируешь файл, решаешь конфликт
git add <файл>
git rebase --continue
```
...
### ЛОКАЛЬНЫЕ ХУКИ (МГНОВЕННАЯ ОБРАТНАЯ СВЯЗЬ — ЛОВЯТ ГЛУПЫЕ ОШИБКИ)

**pre-commit (1–5 секунд)**

- Синтаксические ошибки («забыл скобку!»)
- Форматирование кода («война пробелов и табов»)
- Остаточные `console.log`
- Маркеры конфликтов слияния
- Слишком большие файлы («никаких 100MB видео в репозитории!»)
- Секреты/пароли в коде

**commit-msg (мгновенно)**

- Бессмысленные сообщения коммита («asdfasdf»)
- Отсутствие номера задачи
- Неправильный формат (нет префикса `feat:`, `fix:` и т. д.)

**pre-push (5–30 секунд)**

- Проваленные модульные тесты
- Ошибки линтинга
- Проверка типов (TypeScript/Flow)
- Базовое сканирование безопасности

### CI/CD ПАЙПЛАЙН (МОЩНОСТЬ СЕРВЕРА — ЛОВИТ ПРОБЛЕМЫ ИНТЕГРАЦИИ)

**Этап 1: Быстрая проверка (2–5 минут)**

- Компиляция/сборка («вообще собирается?»)
- Модульные тесты («работает ли код в изоляции?»)
- Покрытие тестами («ты вообще тестировал это?»)
- Проверка зависимостей («этот npm-пакет — не вредоносный?»)

**Этап 2: Интеграция (5–15 минут)**

- Интеграционные тесты («работает ли с базой данных?»)
- Тесты API-контрактов («сломал ли ты работу других команд?»)
- Миграции базы данных («это уничтожит продакшн-данные?»)
- Межсервисное взаимодействие

**Этап 3: Контроль качества (10–30 минут)**

- Регресс производительности («стало ли медленнее?»)
- Сканирование безопасности («есть ли уязвимости?»)
- Нагрузочное тестирование («выдержит ли Чёрную пятницу?»)
- Совместимость с браузерами («работает ли в Safari?»)

**Этап 4: Деплой (5–20 минут)**

- Деплой на staging
- Smoke-тесты («основные функции работают?»)
- Проверка состояния
- Подготовка к откату

**Этап 5: Продакшн (непрерывно)**

- Канарейчный релиз («тест на 1% пользователей»)
- Мониторинг и алерты
- Уровень ошибок
...
**Машина разработчика → Git-сервер → CI-сервер → Staging → Продакшн** 
**БЫСТРО И ДЁШЕВО → → МЕДЛЕННО И ТЩАТЕЛЬНО → РЕАЛЬНЫЙ МИР**

Каждый этап отфильтровывает разные типы проблем:

- **Hooks** — глупые ошибки
- **CI** — ошибки интеграции
- **Staging** — проблемы окружения
- **Продакшн** — реальные пользовательские баги

Чем дальше баг проходит, тем дороже он обходится:

- Пойман хуком → 0 затрат
- Пойман CI → потеря 30 минут
- Пойман на staging → потеря дня
- Пойман в продакшне → убытки + злые пользователи
...
### Многие проекты используют эти хуки:

- **pre-commit.sample** — Запускает проверки перед коммитом (линтинг, тесты, форматирование). **Главная фишка:** не даёт закоммитить мусорный код.
- **commit-msg.sample** — Проверяет сообщения коммита (например, соблюдение правил Conventional Commits). **Главная фишка:** никаких больше тупых «fix bug» коммитов.
- **pre-push.sample** — Запускает проверки перед пушем (полный набор тестов, сканирование безопасности). **Главная фишка:** не даст CI публично тебя опозорить.

### Полезные, но редко настроены:

-  **prepare-commit-msg.sample** — Автоматически заполняет сообщение коммита (добавляет номер задачи, шаблоны). **Главная фишка:** избавляет от рутины при написании коммитов.
- **post-update.sample** — Запускает действия после обновления репозитория (автодеплой, уведомления). **Главная фишка:** автоматизирует скучные задачи после пуша.
- **pre-rebase.sample** — Проверяет безопасность ребейза (предотвращает катастрофы в истории). **Главная фишка:** спасает от ада `git reflog`.

###  Редко используемые (в основном для продвинутых или серверных сценариев):

- **pre-receive.sample** — Серверный хук, проверяет пуши (блокирует плохие коммиты). **Главная фишка:** финальный босс качества кода.
- **fsmonitor-watchman.sample** — Ускоряет Git, отслеживая изменения файлов быстрее. **Главная фишка:** делает Git менее тормозным на больших репозиториях.
- **update.sample** — Срабатывает при обновлении ссылок (гибкий контроль веток). **Главная фишка:** как `pre-receive`, но для отдельных веток.
