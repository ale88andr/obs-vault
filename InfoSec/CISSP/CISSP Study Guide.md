...
|Принцип|Описание|
|---|---|
|**Предположение о взломе (Assume breach)**|Система всегда предполагает, что может быть скомпрометирована|
|**Минимальные привилегии**|Пользователи и процессы получают только необходимые права|
|**Аутентификация и авторизация**|Каждый запрос проверяется на подлинность и разрешения|
|**Контроль доступа на уровне ресурсов**|Доступ предоставляется на максимально детализированном уровне|
|**Непрерывный мониторинг**|Поведение пользователей и устройств постоянно отслеживается|
...
- Только после успешной проверки доступ предоставляется
