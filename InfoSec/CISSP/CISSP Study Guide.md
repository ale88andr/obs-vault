...
### 3.6.3 Инфраструктура открытых ключей (PKI) (например, квантовое распределение ключей)
...
Существуют и другие компоненты и концепции, которые следует знать для экзамена:

PKI может иметь несколько уровней:

- **один уровень** означает, что один или несколько серверов выполняют все функции PKI;
    
- **два уровня** означает, что существует офлайн корневой CA (сервер, который выдаёт сертификаты выдающим CA, но большую часть времени остаётся офлайн) на одном уровне, и выдающие CA (серверы, которые выдают сертификаты вычислительным устройствам и пользователям) — на другом уровне;
    
    - серверы второго уровня часто называют промежуточными CA или подчинёнными CA;
        
- **три уровня** означает, что могут существовать CA, отвечающие только за выдачу политик (и они представляют второй уровень в трёхуровневой иерархии);
    
    - в таком сценарии CA политик также должны оставаться офлайн и включаться только по мере необходимости.
