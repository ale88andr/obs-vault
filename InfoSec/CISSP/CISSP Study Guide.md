...
Атака, при которой злоумышленник имеет доступ только к зашифрованным сообщениям. 
**Суть:** Пытается извлечь информацию или ключ, анализируя только шифртекст.
...
#### 3.7.4 Атака с выбранным открытым текстом (Chosen-plaintext attack)

Атака, при которой злоумышленник может выбрать открытый текст и получить соответствующий шифртекст. 
**Суть:** Позволяет анализировать, как алгоритм шифрует определённые данные.

#### 3.7.5 Атака с выбранным шифртекстом (Chosen-ciphertext attack)

Атака, при которой злоумышленник может выбрать шифртекст и получить соответствующий открытый текст. 
**Суть:** Используется для анализа процесса дешифровки.

#### 3.7.6 Атака по боковому каналу (Side-channel attack)

Атака, использующая физические характеристики системы (время выполнения, энергопотребление, электромагнитные излучения). 
**Суть:** Получает информацию о ключе, не взламывая сам алгоритм.

#### 3.7.7 Атака по времени (Timing attack)

Атака, основанная на измерении времени, необходимого для выполнения криптографических операций. 
**Суть:** Позволяет извлечь ключи, анализируя задержки.

#### 3.7.8 Атака по ошибке (Fault injection attack)

Атака, при которой намеренно вызываются ошибки в системе для получения информации о ключе. 
**Суть:** Использует сбои в аппаратуре или ПО для анализа поведения криптосистемы.

#### 3.7.9 Атака по словарю (Dictionary attack)

Атака, при которой перебираются заранее подготовленные слова или пароли. 
**Суть:** Быстрее, чем brute force, если пароль — распространённое слово.
