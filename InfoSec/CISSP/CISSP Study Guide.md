...
##### CIDR-нотация: компактный способ представления IP-адресов и соответствующих им сетевых масок

**Пример**: `192.168.1.0/24`

Состоит из двух частей:

- **IP-адрес**: `192.168.1.0` — это сетевая или начальная точка адреса
- **/24** — указывает, сколько бит IP-адреса используется для сетевой части; здесь `/24` означает, что первые 24 бита (из 32 для IPv4) используются для сети, а оставшиеся биты — для адресов хостов в этой сети

`/24` эквивалентно маске `255.255.255.0` (где снова 24 бита, представленные как `255.255.255`, определяют сеть, а `.0` — диапазон хостов)

###### Диапазон IP-адресов:

`192.168.1.0/24` представляет сеть `192.168.1.0` и все IP-адреса от `192.168.1.1` до `192.168.1.254`; всего `2^8 = 256` IP-адресов, но **254 из них пригодны для использования** (исключаются сетевой и широковещательный адреса)

###### Другие примеры:

- `10.0.0.0/16`: где `/16` означает, что первые 16 бит зарезервированы для сети, оставляя 16 бит для хостов; позволяет `2^16 = 65,536` IP-адресов, из которых **65,534** пригодны для использования
- `172.16.0.0/12`: `/12` означает, что 12 бит — для сети, оставляя 20 бит для хостов; предоставляет `2^20 = 1,048,576` IP-адресов

###### Классы сетей
- Сеть класса **A** IPv4 содержит **16,777,216** адресов
- Сеть класса **B** содержит **65,534** адреса
- Сеть класса **C** содержит **254** адреса

###### IPSec

Обеспечивает **аутентификацию данных**, **целостность** и **конфиденциальность**

В частности, IPSec предоставляет:
- **Шифрование**
- **Контроль доступа**
- **Неподделываемость (nonrepudiation)**
- **Аутентификацию сообщений** с использованием **криптографии с открытым ключом**

### Безопасные протоколы

**Kerberos**: сетевой протокол аутентификации, основанный на стандартах, используется во многих продуктах (наиболее заметно — в службах домена Active Directory от Microsoft, или AD DS). Kerberos в основном используется в локальных сетях (LAN) для аутентификации в масштабе организации, единого входа (SSO) и авторизации; Kerberos обеспечивает три основные функции: учёт, аутентификацию и аудит.

**SSL и TLS**: используются для защиты транзакций на веб-сайтах (например, банковских операций, электронной коммерции). SSL и TLS обеспечивают **шифрование данных**, **целостность** и **аутентификацию**. TLS вытеснил SSL (оригинальный протокол, считающийся устаревшим/небезопасным). TLS был впервые представлен в 1999 году, но получил широкое распространение лишь спустя несколько лет. Оригинальные версии TLS (1.0 и 1.1) считаются устаревшими, и организациям следует использовать TLS 1.2 или 1.3. Фактическим стандартом для защищённого веб-трафика является **HTTP поверх TLS**, который использует **гибридную криптографию**: асимметричная криптография применяется для обмена эфемерным сеансовым ключом, который затем используется для симметричной криптографии на протяжении всей сессии.

**Безопасный протокол передачи файлов (SFTP)**: версия FTP, которая включает шифрование и используется для передачи файлов между двумя устройствами (часто клиентом и сервером).

**Безопасная оболочка (SSH)**: протокол удалённого управления, который работает поверх TCP/IP. Вся передаваемая информация шифруется. В основном используется ИТ-администраторами для управления такими устройствами, как серверы и сетевое оборудование.
