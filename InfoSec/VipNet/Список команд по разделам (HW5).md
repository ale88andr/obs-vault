# ViPNet Coordinator HW — список команд по разделам
...
- [[#iplir]]
  ...
  - [[#iplir set l2overip]]
  - [[#iplir show]]
  - [[#Прочие команды iplir]]
- [[#machine]]
  - [[#machine backup и config]]
  - [[#machine hosts]]
  - [[#machine logs]]
  - [[#machine reboot]]
  - [[#machine set]]
  - [[#machine show]]
  - [[#Прочие команды machine]]
- [[#mftp]]
- [[#service]]
  - [[#service cert]]
  - [[#service dpi]]
...
- `admin escape` — Выйти в системную командную оболочку shell.

> [!warning] Команда предназначена для использования опытными администраторами в целях отладки. ИнфоТеКС не гарантирует нормальную работу ViPNet Coordinator HW в случае некорректных действий администратора в системной командной оболочке.
...
- `admin show check integrity status` — Просмотреть информацию о последней проверке целостности файлов:
...
- `inet bgp community-list NAME [seq NUM] {permit | deny} COMMUNITY-LINE` — Добавить или изменить правило в комьюнити-листе.
...
- `inet bgp network delete SUBNET netmask NETMASK` — Удалить анонсируемую подсеть.
...
- `inet dgd configuration default` — Сбросить параметры службы DGD на значения по умолчанию:
...
- `inet show mac-address-table [{interface ИНТЕРФЕЙС | address IP-АДРЕС | hwaddress MAC-АДРЕС | vlan ИНТЕРФЕЙС_VLAN}]` — Просмотреть ARP-таблицу (таблицу, содержащую записи о преобразованиях IP-адресов в MAC-адреса).
    > [!note] Если узел недоступен, время жизни записей в ARP-таблице — от 5 до 10 минут.
...
- `inet show usb-modem` — Просмотреть информацию о модеме и настройках подключения к сети текущего оператора.
...
- `inet show wifi` — Просмотреть настройки адаптера Wi-Fi.
    > [!note] Команда доступна только на аппаратных платформах со встроенным адаптером Wi-Fi.
...
- `inet wifi {client | access-point} authentication {open | wpa-psk | wpa2-psk}` — Задать тип защиты сети Wi-Fi:
    - при работе ViPNet Coordinator HW в режиме клиента Wi-Fi;
    - при работе ViPNet Coordinator HW в режиме точки доступа Wi-Fi.
- `inet wifi mode {on | off}` — Включить или выключить интерфейс Wi-Fi.
...
- `iplir show adapters` — Просмотреть все активные статические и динамические сетевые интерфейсы ViPNet Coordinator HW. При просмотре для каждого интерфейса в списке указан параметр allowtraffic, который показывает, разрешено или заблокировано прохождение IP-трафика через интерфейс.
...
- `iplir warning-threshold ПЕРИОД_ОПОВЕЩЕНИЯ` — Задать период оповещения о скором истечении срока действия:
    - ключей обмена;
    - ключа защиты узла;
...
- `machine logs show network-traffic` — Просмотреть журнал IP-пакетов.
...
- `machine set log invalid-packet {on | off}` — Включить или отключить запись нарушений параметров таблицы соединений в системный журнал.
...
- `mftp stop` — Завершить работу службы mftpd (транспортный сервер MFTP).
...
- `service dpi show status` — Просмотреть текущие параметры подсистемы DPI.
...
- `service ips rule update usb` — Обновить базу правил IPS c USB-носителя.
- `service ips show status` — Просмотреть параметры системы предотвращения вторжений IPS.
- `service ips show update-settings` — Просмотреть текущие параметры обновления базы правил IPS.
