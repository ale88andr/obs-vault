...
> Вынесенный подробный разбор механизма проверки (klif.sys, сигнатуры, эмуляция, кэш KSN) — в отдельной заметке [[Как Kaspersky Endpoint Security проверяет файлы]].
...
- Закрыть Kaspersky Endpoint Security командой Выход в контекстном меню иконки в области уведомлений. Этот способ не требует даже повышения привилегий, он доступен любому пользователю.
...
#### 8.4. Какие еще есть настройки защиты

1. Список детектируемых у гроз:
	- Нет причин менять
	- Если есть ложные срабатывания, настройте исключение для конкретных файлов, папок и угроз
2. Действия (всех компонентов):
	- Лечить; если лечить нечего, удалить
	- Нет причин менять
3. Проверять сменные диски при подключении:
	- Включите, защищает от распространения вредоносных программ
4. Использовать локальные задачи
	- Включите, если считаете нужным
5. Не запускать задачи при работе от батареи
	- Отключите для современных ноутбуков
6. Освобождать ресурсы другим программам
	- Нет смысла выключать
7. Уведомлять о событиях
